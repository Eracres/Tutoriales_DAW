...
SELECT DATE_ADD('2024-04-22', INTERVAL FLOOR(RAND() * 100) DAY),
       (SELECT lugar FROM (SELECT lugar FROM botanica ORDER BY RAND() LIMIT 1) AS subquery),
       CONCAT('Nombre', n.n),
       CONCAT('Descripción ', n.n),
       'subidas/hongos_silvestres.jpg'
FROM (SELECT 1 AS n UNION SELECT 2 UNION SELECT 3 UNION SELECT 4 UNION SELECT 5 UNION
      ...
      SELECT 96 UNION SELECT 97 UNION SELECT 98 UNION SELECT 99 UNION SELECT 100) AS n;
